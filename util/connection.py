...
'((SELECT id from \"sever_student\" ss where ss.name = \''+studentname+'\'),\''+str(dt) +'\','+str(engaging)+' )')
        self.conn.commit()
        if(float(engaging) > 0.6):
            self.cur.execute('UPDATE sever_student ss set engagement = TRUE where name = \''+studentname+'\'' )
        
        
        else:
            self.cur.execute('UPDATE sever_student ss set engagement = FALSE where name = \''+studentname+'\'' )
